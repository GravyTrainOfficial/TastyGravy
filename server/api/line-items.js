...
const editUserItem = async (oldItem, quantity, product) => {
  try {
    ...
    return await checkQuantityAsync(newQuantity, async () => {
...
const postUserItem = async (userId, quantity, product, productId) => {
  try {
    ...
    if (possibleOldItem) {
      ...
      return await editUserItem(possibleOldItem, quantity, product)
    }
    return await checkQuantityAsync(quantity, async () => {
      try {
...
const editGuestItem = async (oldItem, quantity) => {
  const newQuantity = oldItem.quantity + quantity

  return await checkQuantityAsync(newQuantity, () => {
...
const postGuestItem = async (cart = [], quantity, product, productId) => {
  ...
  if (possibleOldItem) {
    console.log('oh no! this already exists! switching to edit...')
    return await editGuestItem(possibleOldItem, quantity)
  }
  return await checkQuantityAsync(quantity, () => {
...
router.put('/', async (req, res, next) => {
  try {
    ...
    if (req.user) {
      ...
      result = await editGuestItem(oldItem, quantity)
    }
    console.log(result)
    if (result === 'forbidden') res.status(403).send()
    else if (result === 'delete') return await deleteItem(req, res, next)
